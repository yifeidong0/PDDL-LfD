...
# Robots
MODEL_DIRECTORY = join_paths(get_parent_dir(__file__), 'models/')
PANDA_HAND_URDF = join_paths(MODEL_DIRECTORY, 'franka_description/robots/hand.urdf')
PANDA_ARM_URDF = join_paths(MODEL_DIRECTORY, 'franka_description/robots/panda_arm.urdf')
PANDA_ARM_HAND_URDF = join_paths(MODEL_DIRECTORY, 'franka_description/robots/panda_arm_hand.urdf')
...
def vertices_from_rigid(body, link=BASE_LINK):
    # This ensures the body is an object with 1 link
    # Commented this for manipulating blocks with character
...
def plan_joint_motion(body, joints, end_conf, obstacles=[], attachments=[],
    ...
    start_conf = get_joint_positions(body, joints)
    # TODO disabled end-conf collision checking
...
def plan_joint_motion_interpolation(body, joints, end_conf, obstacles=[], attachments=[],
                      self_collisions=True, disabled_collisions=set(),
                      weights=None, resolutions=None, max_distance=MAX_DISTANCE,
                      use_aabb=False, cache=True, custom_limits={}, algorithm=None, **kwargs):
    ...
    # path is the interpolation of start conf and end conf with 70 steps
    path = interpolate_joint_waypoints(body, joints, [start_conf, end_conf], 
                                       resolutions=resolutions, **kwargs)
    return path
...
def sample_placement_pos_on_aabb(top_body, bottom_aabb,
                             percent=1.0, max_attempts=50, epsilon=1e-3):
    for _ in range(max_attempts):
        center, extent = get_center_extent(top_body)
...
def sample_placement_pos(top_body, bottom_body, bottom_link=None, **kwargs):
    bottom_aabb = get_aabb(bottom_body, link=bottom_link)
    return sample_placement_pos_on_aabb(top_body, bottom_aabb, **kwargs)

def sample_placement_reachable(top_body, bottom_body, bottom_link=None, 
                               top_pose=unit_pose(),  epsilon=1e-3,
                               max_attempts=50, **kwargs):
    reach_range = kwargs.get("reach_range", (0.25, 1.0))
    reach_theta = kwargs.get("reach_theta", (-np.pi, np.pi))
    ...
    for _ in range(max_attempts):
        ...
        radius = np.random.uniform(*reach_range)
        x, y = radius*unit_from_theta(np.random.uniform(reach_theta[0], 
                                                        reach_theta[1]))
        z = stable_z(top_body, bottom_body)
        point = np.array([x, y, z]) + (get_point(top_body) - center)
        pose = multiply(Pose(point, rotation), top_pose)
        # set_pose(top_body, pose)
        return pose
...
def sample_placement_reachable_pos(top_body, bottom_body, 
                                max_attempts=50, 
                                **kwargs):
    reach_range = kwargs.get("reach_range", (0.25, 1.0))
    reach_theta = kwargs.get("reach_theta", (-np.pi, np.pi))
    for _ in range(max_attempts):
        center, extent = get_center_extent(bottom_body)
        radius = np.random.uniform(*reach_range) * extent[0]/2 
        x, y = radius*unit_from_theta(np.random.uniform(reach_theta[0], 
                                                        reach_theta[1]))
        z = stable_z(top_body, bottom_body)
        point = np.array([x+center[0],
                        y+center[1],
                        z]) 
        return point
    return None
...
def inverse_kinematics(robot, link, target_pose, 
    ...
    for iteration in irange(max_iterations):
        ...
        return None
    # TODO disabled joint limits
    lower_limits, upper_limits = get_custom_limits(robot, movable_joints, custom_limits)
    if not all_between(lower_limits, kinematic_conf, upper_limits):
        return None
    return kinematic_conf

def inverse_kinematics_tracik(robot, link, target_pose,
                              max_iterations=200, max_time=INF,
                              custom_limits={}, **kwargs):
    base_link = "panda_link0"
    tool_link = "tool_link"
    ik_sover = TracIKSolver(PANDA_ARM_URDF,
                            base_link,
                            tool_link)
    movable_joints = get_movable_joints(robot)
    target_matrix = np.eye((4))
    target_matrix[:3, 3] = target_pose[0]
    target_matrix[:3, :3] = matrix_from_quat(target_pose[1])
    for _ in irange(max_iterations):
        qinit = get_joint_positions(robot, movable_joints)[:7]
        kinematic_conf = ik_sover.ik(target_matrix,
                                    qinit=qinit)
        if kinematic_conf is None:
            return None
        set_joint_positions(robot, movable_joints, kinematic_conf)
