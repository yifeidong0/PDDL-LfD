...
class HookWorld():
	...
	def load_hooks(self, hooks_info):
		init_hook_states = []
		with HideOutput():
	...
	def template_load_objects(self, target_objects_info):
		# This function load every objects on the table
		# use the random_load_objects with max_stack_num = 0
		init_obj_states = self.random_load_objects(target_objects_info, 
													max_stack_num=1)
		return init_obj_states

	def random_load_objects(self, target_objects_info, max_stack_num=None,
							):
		init_pose = Pose(Point(x=0, y=0, z=TABLE_HEIGHT+BLOCK_DIMS[2]/2))
		block_ids = []
		for name, block_info in target_objects_info.items():
		...
		points = sampler.make_samples(num=num_target_objects*10, filter=filter)
		
		target_objects = list(target_objects_info.keys())
		stacking = self.make_random_stacking(target_objects, max_stack_num)
		...
		for stack in stacking:
			...
			target_objects_info[stack[0]]["pose"] = point.copy()
			block_pose = BodyPose(target_objects_info[stack[0]]["id"], 
								Pose(Point(x=target_objects_info[stack[0]]["pose"][0],
											y=target_objects_info[stack[0]]["pose"][1],
											z=target_objects_info[stack[0]]["pose"][2]
											)))
	...
	def get_logical_state(self, robots_info, tables_info, target_objects_info):
		...
		obj_infos = [[info["id"]]+list(get_point(info["id"])) for _, info in target_objects_info.items()]
	...
	def make_random_stacking(self, target_objects, max_stack_num=None, 
		...
		block_perm = target_objects.copy()
		...
		if max_stack_num is not None:
			assert (
				0 < max_stack_num <= num_target_objects
			), "Max stack height must be a integer greater than 0 and less than the number of target_objects"
			num_max_stacks = num_target_objects//max_stack_num
		...
		# Generate random stacking
			...
			elif num_target_objects == 1:
				return set([tuple(block_perm)]) | stacking

			if num_stacks is None:
				num_splits = np.random.randint(lower_num, num_target_objects)
			else:
				assert num_stacks >= 0 and num_stacks < num_target_objects, "Invalid stack number"
				num_splits = num_stacks - 1
			split_locs = np.random.choice(
				list(range(1, num_target_objects)), size=num_splits, replace=False
			)
			split_locs.sort()
			split_locs = np.append(split_locs, num_target_objects)
			i = 0
	...
	def load_world(self, robots_info, tables_info, target_objects_info,
		...
		if template:
			if with_pose:
				init_block_states = self.template_load_objects_with_pose(target_objects_info)
			else:
				init_block_states = self.template_load_objects(target_objects_info, 
										table_id=tables_info["floor"]["id"])
		else:
			init_block_states = self.random_load_objects(target_objects_info,
	...
	def template_load_objects_with_pose(self, target_objects_info):
		init = []
		with HideOutput():
			for name, object_info in target_objects_info.items():
	...
	def grasp(self, robot_id, obj_id, target_pos):
		# Moves the robot's end-effector (EEF) to the target position near the object.
	...
	def reset(self, robots_info, target_objects_info):
		for name, robot_info in robots_info.items():
